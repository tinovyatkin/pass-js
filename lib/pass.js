...
Pass.prototype.getPassJSON = function() {
  const fields = cloneObject(this.fields);
...
Pass.prototype.pipe = function(output) {
  ...
  // Create pass.json
  const passJson = new Buffer(JSON.stringify(this.getPassJSON()), 'utf-8');
  ...
  for (let i = 0; i < langs.length; i++) {
    ...
    addFile(`${lang}.lproj/pass.strings`).end(
      new Buffer(this.localizations[lang]),
      'utf-16',
    );
  }
  ...
  for (const key in this.images) {
    ...
    addImage(addFile(filename), this.images[key], error => {
      --expecting;
  ...
  function doneWithImages() {
    if (lastError) {
      ...
      process.nextTick(() => {
        self.signZip(zip, manifest, error => {
          if (error) {
...
function addImage(file, source, callback) {
  if (typeof source === 'string' || source instanceof String) {
    if (/^https?:/i.test(source)) {
      // URL
      const protocol = /^https:/i.test(source) ? HTTPS : HTTP;
      protocol
        .get(source, response => {
          if (response.statusCode == 200) {
            ...
            callback(
              new Error(`Server returned ${response.statusCode} for ${source}`),
            );
        })
        .on('error', callback);
    } else {
      // Assume filename
      const stream = File.createReadStream(source);
      stream.pipe(file);
      file.on('close', callback);
    }
    ...
    callback(
      new Error(
        `Cannot load image ${file.filename}, must be String (filename), Buffer or function`,
      ),
    );
  }
}
...
Pass.prototype.signZip = function(zip, manifest, callback) {
  ...
  // Create signature
  signManifest(this.template, json, (error, signature) => {
    if (!error) {
...
function signManifest(template, manifest, callback) {
  const identifier = template.passTypeIdentifier().replace(/^pass./, '');
  ...
  const args = [
    ...
    '-binary',
    '-signer',
    Path.resolve(template.keysPath, `${identifier}.pem`),
    '-certfile',
    Path.resolve(template.keysPath, 'wwdr.pem'),
    '-passin',
...
function cloneObject(object) {
  const clone = {};
  if (object) {
    for (const key in object) clone[key] = object[key];
  }
  return clone;
}
