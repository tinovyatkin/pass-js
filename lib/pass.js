...
Pass.prototype.addLocalization = function (lang, values) {
  ...
  var strings = keys.map(function(key) {
    return "\"" + key + "\" = \"" + values[key].replace(/"/g, "\\\"") + "\";";
  });
...
Pass.prototype.pipe = function(output) {
  ...
  for (var i = 0; i < langs.length; i++) {
    var lang = langs[i];
    addFile(lang + ".lproj/pass.strings").end(new Buffer(this.localizations[lang]), 'utf-16');
  }

  var expecting = 0;
  for (var key in this.images) {
    var filename = key.replace(/2x$/, "@2x").replace(/3x$/, "@3x") + ".png";
