...
File.prototype.write = function(buffer, encoding) {
  console.log(this.filename, buffer.length)
  if (!this.writable)
...
File.prototype._writeDataDescriptor = function() {
  ...
  write32(buffer, 0x08074b50, 0);
  write32(buffer, this._crc ^ -1, 4);
  // compressed size
  write32(buffer, this._compressedLength, 8);
  // uncompressed size
  write32(buffer, this._uncompressedLength, 12);
  this.zip._writeBuffer(buffer);
};
