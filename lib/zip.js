...
Zip.prototype._writeCentralDirectoryHeader = function(file) {
  const filename = new Buffer(file.filename, 'utf-8');
  const buffer = new Buffer(46 + filename.length);
...
Zip.prototype._writeEndOfCentralDirectory = function(
...
File.prototype.write = function(buffer, encoding) {
  ...
  for (let i = 0; i < buffer.length; i++) {
    const offset = (this._crc ^ buffer[i]) & 0xff;
...
File.prototype._writeLocalFileHeader = function() {
  ...
  const filename = new Buffer(this.filename, 'utf-8');
  const buffer = new Buffer(30 + filename.length);
