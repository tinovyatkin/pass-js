...
// Generate Zip file. Returns a buffer.
Zip.prototype.generate = function() {
  ...
  for (var i in this.files) {
    var file = this.files[i];
    var header = centralDirectoryHeader(file, offset);
  ...
  buffers.push(endOfCentralDirectoryRecord(this.files, offset, size));
...
function localFileHeader(file) {
  var compressed = file.content;
  var filename = new Buffer(file.name, "utf-8");
  var buffer = new Buffer(30 + filename.length + compressed.length);
  ...
  write16(buffer, 0x130A, 4);
  // general purpose bit flag  
  write16(buffer, 0x0004, 6);
  // compression method
  write16(buffer, 0x0000, 8); // STORE
  // last mod file time
  write16(buffer, getTimePart(file.modified), 10);
  // last mod file date
  write16(buffer, getDatePart(file.modified), 12);
  // crc-32
  write32(buffer, crc32(compressed), 14);
  // compressed size
  write32(buffer, compressed.length, 18); // no compression
  // uncompressed size
  ...
  // File data
  compressed.copy(buffer, 30 + filename.length);
...
function centralDirectoryHeader(file, offset) {
  ...
  // crc-32
  write32(buffer, crc32(compressed), 16);
  // compressed size
  write32(buffer, compressed.length, 20); // no compression
  // uncompressed size
