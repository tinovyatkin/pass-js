 ...
 */
async function signManifestFromPath(signerPemFile, password, manifest) {
	// reading and parsing certificates
	const signerCertData = await readFile(signerPemFile, 'utf8');
	return signManifest(signerCertData, password, manifest);
}

async function signManifestFromVar(cert, password, manifest) {
	return signManifest(cert, password, manifest);
}

function signManifest(signerCertData, password, manifest) {
...
module.exports = {signManifestFromPath, signManifestFromVar};
